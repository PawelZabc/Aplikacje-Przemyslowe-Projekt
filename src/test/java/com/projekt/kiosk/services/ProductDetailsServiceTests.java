...
class ProductDetailsServiceTests {
        ...
        @Test
        void getProductDetails_returnsEmptyLists_whenNoIngredientsOrExtras() {
                // given
                ProductEntity product = new ProductEntity();
                product.setId(2);
                product.setName("Water");
                product.setPriceCents(200);

                when(productRepository.findById(2)).thenReturn(Optional.of(product));
                when(productIngredientRepository.findByProductId(2)).thenReturn(List.of());
                when(productExtraRepository.findByProductId(2)).thenReturn(List.of());

                // when
                ProductDetailsDto result = productDetailsService.getProductDetails(2);
